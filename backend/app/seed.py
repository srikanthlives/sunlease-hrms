"""Seed initial roles + permission grants + logins, and (in "full" mode
only) a demo org structure + a testable HR_STAFF/APPROVER pair with Cost
Center scope, so the RBAC and approval-routing flow (blueprint §15/§18)
is exercisable immediately after a fresh seed. Safe to re-run (idempotent).

HRMS_SEED_MODE controls how much gets created:
  - "full" (default) - everything below, including sample Company/Cost
    Center/Department/Categories/etc., hrstaff/approver test logins, and
    Module 2/3 demo master data (shifts, leave types, salary components,
    statutory config). Good for local dev/demo.
  - "minimal" - ONLY the 5 Role rows (so HR_STAFF/APPROVER/EMPLOYEE are
    still assignable to real users later) + their default permission
    grants + the two admin logins (admin/HR_ADMIN, superadmin/
    SUPER_ADMIN). No sample org structure, no test users beyond the two
    admins, no demo shifts/leave types/salary components/statutory
    config - HR Admin creates all of that themselves through the app.
    Use this for a real production deployment:
...
SEED_MODE = os.environ.get("HRMS_SEED_MODE", "full").strip().lower()
if SEED_MODE not in ("full", "minimal"):
    print(f"[SEED] Unknown HRMS_SEED_MODE={SEED_MODE!r}, defaulting to 'full'")
    SEED_MODE = "full"
print(f"[SEED] Running in {SEED_MODE!r} mode")
...
try:
    ...
    # SUPER_ADMIN - exclusively for the database backup/restore tools
    # (routers/admin.py); a superset of HR_ADMIN everywhere HR_ADMIN is
    # allowed, but HR_ADMIN itself cannot reach db-backup/db-restore.
    ...
    db.commit()
    print(f"Seed complete ({SEED_MODE} mode).")
    if SEED_MODE == "full":
        print("Logins: admin / Admin@123, hrstaff / HrStaff@123, approver / Approver@123, superadmin / SuperAdmin@123")
    else:
        print("Logins: admin / Admin@123, superadmin / SuperAdmin@123")
        print("No sample org data/master data/test users seeded - create your own via the app.")
finally:
    db.close()
