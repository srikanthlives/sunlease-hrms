...
export default function LeaveApplications() {
  ...
  function reloadForEmployee() {
    if (!episodeId) return;
    setError("");
    const params = { episode_id: episodeId, year, month };
    if (costCenterId) params.cost_center_id = costCenterId;
    client.get(`/leave/balances/${episodeId}`).then((res) => setBalances(res.data)).catch((err) => setError(apiErrorMessage(err)));
    client.get("/leave/applications", { params }).then((res) => setApplications(res.data)).catch((err) => setError(apiErrorMessage(err)));
  }

  useEffect(reloadForEmployee, [episodeId, year, month, costCenterId]);
  ...
  async function applyLeave() {
    ...
    setSubmitting(true);
    try {
      const res = await client.post("/leave/applications", {
      ...
      if (attachmentFile) {
        ...
        await client.post(`/leave/applications/${res.data.id}/attachment`, formData, { headers: { "Content-Type": "multipart/form-data" } });
      }
  ...
  async function uploadAttachment(applicationId, file) {
    ...
    try {
      ...
      await client.post(`/leave/applications/${applicationId}/attachment`, formData, { headers: { "Content-Type": "multipart/form-data" } });
      reloadForEmployee();
    } catch (err) {
  ...
  async function downloadAttachment(applicationId, fileName) {
    setError("");
    try {
      const res = await client.get(`/leave/applications/${applicationId}/attachment`, { responseType: "blob" });
  ...
  async function cancelApplication(id) {
    ...
    try {
      await client.post(`/leave/applications/${id}/cancel`);
      reloadForEmployee();
    } catch (err) {
      setError(apiErrorMessage(err));
    }
  ...
  const canApply = episodeId && form.leave_type_id && form.start_date && (form.is_half_day || form.end_date);

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-display font-semibold text-ink">Leave Applications</h1>
        <p className="text-sm text-ink/50 mt-1">Apply leave on behalf of an employee, view balances, and cancel pending/upcoming applications.</p>
      </div>
      ...
      {episodeId && (
        <>
          <Card>
            <h2 className="text-sm font-semibold text-ink mb-3">Leave Balances{selectedEmployee ? ` — ${selectedEmployee.first_name} ${selectedEmployee.last_name}` : ""}</h2>
